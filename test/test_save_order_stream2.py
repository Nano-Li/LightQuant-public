...
def print_message(conn: Connection, response: WebSocketResponse):
    ...
    with open('order_data_SHIB.json', 'a') as f:
...
async def main():
    ...
    if not Path('order_data_SHIB.json').is_file():
        with open('order_data_SHIB.json', 'w') as f:
            f.write("[\n")
    ...
    channel.subscribe(['account_number', 'SHIB_USDT'])
