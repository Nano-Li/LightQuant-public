            ...
            if response.channel == 'futures.order_place':
                ...
                for _i, each_order_result in enumerate(response.result):
                    ...
                    if each_order_result.get('succeeded'):
                        ...
                        if each_order_result.get('label'):
                            if each_order_result.get('detail'):
                                single_ack_status['error'] = GateWebsocketApiError(each_order_result['label'], each_order_result['detail'])
                            else:
                                single_ack_status['error'] = GateWebsocketApiError(each_order_result['label'], 'No_Detail')
                        # todo: 可能存在位置的batchorder错误返回，因此导致服务器报错
                        else:
                            single_ack_status['error'] = GateWebsocketApiError('UNIDENTIFIED', 'UNIDENTIFIED')
                            print(f'未知的 async batch order 错误返回: \n {each_order_result}')
