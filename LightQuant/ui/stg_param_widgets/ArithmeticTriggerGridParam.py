# -*- coding: utf-8 -*-
# @Time : 2023/4/17 21:36
# @Author : 
# @File : ArithmeticTriggerGridParam.py 
# @Software: PyCharm
...
class ArithmeticTriggerGridParam(ParamWidget):
    """
    高级等差网格的参数输入窗口
    """
    ...
    def __init__(self):
        ...
        self.label_trigger_condition: QLabel = None
        ...
        self.select_price_trigger: QCheckBox = None
        self.select_entry_smart: QRadioButton = None
        self.select_entry_maker: QRadioButton = None
        self.select_entry_taker: QRadioButton = None
        ...
        self.input_price_trigger: QLineEdit = None
    ...
    def _init_setting(self):
        ...
        self.setWindowTitle('条件触发网格参数')

        main_frame = QFrame(self)
        main_frame.setGeometry(10, 10, 380, 840)
        ...
        # ====================================== 触发条件 ====================================== #
        self.label_trigger_condition = QLabel('触发条件', main_frame)
        self.label_trigger_condition.setGeometry(60, 310, 60, 24)

        frame_trigger_condition = QFrame(main_frame)
        frame_trigger_condition.setGeometry(20, 320, 340, 161)
        frame_trigger_condition.setFrameStyle((QFrame.Box | QFrame.Plain))
        frame_trigger_condition.setObjectName('frame_trigger_condition')
        frame_trigger_condition.setStyleSheet("QFrame#frame_trigger_condition{border-radius: 0px}")
        frame_trigger_condition.setEnabled(True)
        frame_trigger_condition.lower()

        self.select_pending_trigger = QCheckBox('挂单触发', frame_trigger_condition)
        self.select_pending_trigger.setFont(temp_font)
        self.select_pending_trigger.setGeometry(radio_x, 20, 108, 24)
        self.select_pending_trigger.setChecked(True)
        self.select_pending_trigger.stateChanged.connect(
            lambda: (self.input_pending_trigger.setEnabled(True),
                     self.select_price_trigger.setChecked(False),
                     self._disable_confirm_btn())
        ...
        self.input_pending_trigger = QLineEdit(frame_trigger_condition)
        self.input_pending_trigger.setFont(temp_font)
        self.input_pending_trigger.setGeometry(170, 20, 150, 24)
        self.input_pending_trigger.setEnabled(True)
        self.input_pending_trigger.textEdited.connect(self._disable_confirm_btn)

        self.select_price_trigger = QCheckBox('价格触发', frame_trigger_condition)
        self.select_price_trigger.setFont(temp_font)
        self.select_price_trigger.setGeometry(radio_x, 50, 108, 24)
        self.select_price_trigger.setChecked(False)
        self.select_price_trigger.stateChanged.connect(
            lambda: (self.input_price_trigger.setEnabled(True),
                     self.select_entry_smart.setEnabled(True),
                     self.select_entry_maker.setEnabled(True),
                     self.select_entry_taker.setEnabled(True),
                     self.select_pending_trigger.setChecked(False),
                     self._disable_confirm_btn())
            if self.select_price_trigger.isChecked()
            else (self.input_price_trigger.setEnabled(False),
                  self.select_entry_smart.setEnabled(False),
                  self.select_entry_maker.setEnabled(False),
                  self.select_entry_taker.setEnabled(False),
                  self._disable_confirm_btn())
        )
        # todo: 该功能暂时不开放，后续添加功能时加入
        self.select_price_trigger.setEnabled(False)

        self.input_price_trigger = QLineEdit(frame_trigger_condition)
        self.input_price_trigger.setFont(temp_font)
        self.input_price_trigger.setGeometry(170, 50, 150, 24)
        self.input_price_trigger.setEnabled(False)
        self.input_price_trigger.textEdited.connect(self._disable_confirm_btn)

        radio_x2 = 32
        temp_font.setPixelSize(12)
        self.select_entry_smart = QRadioButton('智能 Maker 进场', frame_trigger_condition)
        self.select_entry_smart.setFont(temp_font)
        self.select_entry_smart.setGeometry(radio_x2, 80, 128, 24)
        self.select_entry_smart.setEnabled(False)
        self.select_entry_maker = QRadioButton('强制 Maker 进场', frame_trigger_condition)
        self.select_entry_maker.setFont(temp_font)
        self.select_entry_maker.setGeometry(radio_x2, 100, 128, 24)
        self.select_entry_maker.setEnabled(False)
        self.select_entry_taker = QRadioButton('强制 Taker 进场', frame_trigger_condition)
        self.select_entry_taker.setFont(temp_font)
        self.select_entry_taker.setGeometry(radio_x2, 120, 128, 24)
        self.select_entry_taker.setEnabled(False)
        temp_font.setPixelSize(16)

        # ====================================== 高级选项 ====================================== #
        self.enable_advance_param = QCheckBox('高级选项', main_frame)
        self.enable_advance_param.setGeometry(60, 470, 70, 24)
        ...
        self.frame_advance_param.setGeometry(20, 480, 340, 151)
        ...
        self.select_target_price.setChecked(False)
        self.select_target_price.stateChanged.connect(
        ...
        self.enable_stop_loss_param.setGeometry(60, 620, 70, 24)
        ...
        self.frame_stop_loss_param.setGeometry(20, 630, 340, 91)
        ...
        frame_buttons.setGeometry(20, 720, 340, 100)
    ...
    def _update_widget_status(self) -> None:
        ...
        if self.select_side_mid.isChecked():
            ...
            self.select_pending_trigger.setChecked(False)
            self.select_pending_trigger.setEnabled(False)
        else:
            self.select_target_ratio.setEnabled(True)
            self.select_target_price.setEnabled(True)

            self.select_pending_trigger.setEnabled(True)
    ...
    def check_inputs(self) -> dict:
        ...
        if (not self.select_pending_trigger.isChecked()) and not (self.select_price_trigger.isChecked()):
            all_okay = False
            error_msg += '请选择触发条件!\n\n'
    ...
    def remove_redundant(self) -> None:
        ...
        if not self.select_pending_trigger.isChecked():
            self.input_pending_trigger.clear()

        if not self.select_price_trigger.isChecked():
            self.input_price_trigger.clear()
    ...
    def get_input_params(self) -> dict:
        ...
        input_param_dict['trigger_selection'] = self._get_trigger_selection()
        ...
        input_param_dict['price_trigger_price'] = float(self.input_price_trigger.text()) if self.select_price_trigger.isChecked() else ''
    ...
    def update_text(self, updated_params: dict) -> None:
        ...
        self.input_price_trigger.setText(str(updated_params['price_trigger_price']))
    ...
    def _get_trigger_selection(self) -> int:
        if self.select_pending_trigger.isChecked():
            return 1
        elif self.select_price_trigger.isChecked():
            return 2
        else:
            return 0
