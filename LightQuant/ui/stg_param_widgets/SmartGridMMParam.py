...
class SmartGridMarketMakerParam(ParamWidget):
    """
    智能做市网格参数
    """
    ...
    def __init__(self):
        ...
        self.input_short_ratio: QLineEdit = None
        self.input_account_num: QLineEdit = None

        self._init_setting()

    def _init_setting(self):
        # self.setWindowFlags(Qt.WindowStaysOnTopHint)    # 测试用功能
        self.setFixedSize(400, 550)  # 增加了窗体的高度以适应更多的输入行
        self.setWindowTitle('智能高频做市网格参数')

        main_frame = QFrame(self)
        main_frame.setGeometry(10, 10, 380, 530)
        ...
        frame_function_param.setGeometry(20, 170, 340, 181)  # 根据需要调整位置和大小
        ...
        # 网格间距
        label_grid_abs_step = QLabel('网格间距', frame_function_param)
        label_grid_abs_step.setFont(temp_font)
        label_grid_abs_step.setGeometry(label_x, 20, label_width, row_height)
        self.input_grid_abs_step = QLineEdit(frame_function_param)
        self.input_grid_abs_step.setFont(temp_font)
        self.input_grid_abs_step.setGeometry(input_x, 20, input_width, row_height)
        ...
        # 做空比例
        label_short_ratio = QLabel('做空比例', frame_function_param)
        label_short_ratio.setFont(temp_font)
        label_short_ratio.setGeometry(label_x, 140, label_width, row_height)
        self.input_short_ratio = QLineEdit(frame_function_param)
        self.input_short_ratio.setFont(temp_font)
        self.input_short_ratio.setGeometry(input_x, 140, input_width, row_height)
        self.input_short_ratio.textEdited.connect(self._disable_confirm_btn)

        # ====================================== 账号参数 ====================================== #
        label_account_param = QLabel('子账号', main_frame)
        label_account_param.setGeometry(50, 340, 45, 24)

        frame_function_param = QFrame(main_frame)
        frame_function_param.setGeometry(20, 350, 340, 61)
        frame_function_param.setFrameStyle((QFrame.Box | QFrame.Plain))
        frame_function_param.setObjectName('frame_function_param')
        frame_function_param.setStyleSheet("QFrame#frame_function_param{border-radius: 0px}")
        frame_function_param.lower()
        ...
        # 做空比例
        label_account_num = QLabel('账号数量', frame_function_param)
        label_account_num.setFont(temp_font)
        label_account_num.setGeometry(label_x, 20, label_width, row_height)
        self.input_account_num = QLineEdit(frame_function_param)
        self.input_account_num.setFont(temp_font)
        self.input_account_num.setGeometry(input_x, 20, input_width, row_height)
        self.input_account_num.textEdited.connect(self._disable_confirm_btn)

        # ====================================== 按钮部分 ====================================== #
        frame_buttons = QFrame(main_frame)
        frame_buttons.setGeometry(20, 410, 340, 100)
    ...
    def check_inputs(self) -> dict:
        ...
        input_short_ratio = self.input_short_ratio.text()
        input_account_num = self.input_account_num.text()
        ...
        # 检查合约杠杆
        if not self._valid_num_or_empty(input_leverage):
            all_okay = False
            error_msg += '合约杠杆输入错误\n\n'
        ...
        # 检查比例间距
        if not self._valid_num_or_empty(input_grid_abs_step):
            all_okay = False
            error_msg += '比例间距输入错误\n\n'
        ...
        # 检查做空比例
        if not self._valid_num_or_empty(input_account_num):
            all_okay = False
            error_msg += '做空比例输入错误\n\n'
    ...
    def get_input_params(self) -> dict:
        ...
        input_param_dict['leverage'] = '' if self.input_leverage.text() == '' else int(self.input_leverage.text())
        ...
        # input_param_dict['grid_abs_step'] = '' if self.input_grid_abs_step.text() == '' else float(self.input_grid_abs_step.text())
        ...
        #
        # input_param_dict['short_ratio'] = float(self.input_short_ratio.text()) if self.input_short_ratio.text() else 0.0
        # input_param_dict['account_num'] = int(float(self.input_account_num.text()))

        input_param_dict['symbol_name'] = 'BTC_USDT'
        input_param_dict['up_price'] = 74500
        # input_param_dict['leverage'] = 50
        input_param_dict['trigger_start'] = False
        input_param_dict['pending_trigger_price'] = ''
        # 对于比例间距、补仓价差和下方价差，如果输入为空，则保留空字符串，否则转换为float
        input_param_dict['grid_abs_step'] = 1
        input_param_dict['filling_step'] = 0.6
        input_param_dict['lower_step'] = 0.9
        input_param_dict['filling_fund'] = 0.0
        input_param_dict['short_ratio'] = 0.5
        input_param_dict['account_num'] = 5
    ...
    def update_text(self, updated_params: dict) -> None:
        ...
        self.input_short_ratio.setText(str(updated_params['short_ratio']))
        self.input_account_num.setText(str(updated_params['account_num']))
