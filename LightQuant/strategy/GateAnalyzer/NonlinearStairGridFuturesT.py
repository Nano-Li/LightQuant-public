...
# @Time : 2023/5/8 21:37
# @Author : 
# @File : NonlinearStairGridFuturesT.py 
# @Software: PyCharm
import time
import asyncio
import numpy as np
...
class NonlinearStairGridAnalyzerFuturesT(Analyzer):
    """
    ...
    async def _start_strategy(self):
        ...
        max_allowed_order_num = int(self.current_symbol_price * 0.5 / self.grid_price_step)
        ...
        if self._is_trading:
            if self.all_grid_price[self.present_bottom_index] < self.current_symbol_price < self._x_grid_up_limit:
                ...
                if self.critical_index >= 1:
                    if abs(calc(self.current_symbol_price, self.all_grid_price[self.critical_index], '-')) * 0.8 > \
                            calc(self.all_grid_price[self.critical_index], self.all_grid_price[self.critical_index - 1], '-'):
