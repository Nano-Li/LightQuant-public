# -*- coding: utf-8 -*-
# @Time : 2023/3/21 12:21
# @Author : 
# @File : MakerInOutMargin.py
import time
import asyncio
import pandas as pd
...
class MakerInOutAnalyzerMargin(Analyzer):
    """
    测试用策略，使用maker不断进出市场，尝试赚取负手续
    """
    ...
    def __init__(self) -> None:
        ...
        # 设置买卖单最大挂单数量，及最小数量
        self.max_buy_order_num = 12
        self.max_sell_order_num = 30
        self.min_buy_order_num = 5
        ...
        self._exist_spot_position: bool = True          # True 代表市价买多完成，False代表卖空清仓
        self._need_next_order:bool = False              # 需要下一次开单
        self._interval_count: int = 0                   # 买卖单操作间隔
        ...
        self._previous_avg_deal_price = 0
    ...
    async def param_ref_info(self, valid_param_dict: dict) -> str:
        ...
        init_account_spot_requirement = calc((self.max_buy_order_num + 3), self.grid_each_qty, '*')
    ...
    async def _start_strategy(self):
        ...
        # asyncio.create_task(self._test_maker())     # todo: turn back
    ...
    async def _test_maker(self) -> None:
        """
        todo: 该函数用于测试 在最新价下挂maker单
        :return:
        """
        if self._maker_market_lock:
            self._log_info('存在maker市价单任务，不开启新任务')
            return
        self._maker_market_lock = True

        self._log_info('测试maker市价单功能')
        self._trading_statistics['strategy_start_time'] = self.gen_timestamp()
        ...
        maker_order['id'] = self.gen_id(99999998, self.BUY)     # todo: 此处给一个错误，用于测试代码热加载
        maker_order['price'] = calc(self.current_symbol_price, 2*self.symbol_quantity_min_step, '-')
        maker_order['side'] = self.BUY
        # maker_order['quantity'] = calc(self.symbol_quantity_min_step, 10, '*')
        self._log_info(str(maker_order['id']))
        ...
        adding_volume = calc(self.filling_quantity, self.current_symbol_price, '*')
    ...
    async def _layout_net(self):
        ...
        await asyncio.sleep(0.6)  # todo: 此处等待买入完成，才能在上方挂卖单，可以测试该时间
    ...
    async def _maintain_grid_order(self, data_dict: dict, append_info: str = None) -> None:
        ...
        if this_order_index == 99999999:
        ...
        elif this_order_index == 99999998:
            # todo: 测试用id
            ...
            self._trading_statistics['total_trading_fees'] = calc(self._trading_statistics['total_trading_fees'], calc(adding_volume, self.symbol_maker_fee, '*'), '+')
            self._finished_maker_value = calc(self._finished_maker_value, adding_volume, '+')
            avg_deal_price = calc(self._finished_maker_value, self.filling_quantity, '/')
            self._log_info('成交均价:\t{}'.format(avg_deal_price))

            if self._target_maker_side == self.SELL:
                self._trading_statistics['filled_sell_order_num'] += 1
                profit = calc(calc(avg_deal_price, self._previous_avg_deal_price, '-'), self.filling_quantity, '*')
                self._log_info('\n==============================')
                self._log_info('本轮进出收益:\t{}\tUSDT'.format(profit))
                self._log_info('==============================\n')
                self._trading_statistics['in_out_profit'] = calc(self._trading_statistics['in_out_profit'], profit, '+')
            else:
                self._trading_statistics['filled_buy_order_num'] += 1
                self._previous_avg_deal_price = avg_deal_price
            ...
            if self._target_maker_side == self.BUY:
                self._exist_spot_position = True
            else:
                self._exist_spot_position = False
            self._need_next_order = True
            return
    ...
    async def _terminate_maker_market(self) -> None:
        ...
        while True:
            ...
            if (not self._maker_market_switch_on) and (not self._exist_spot_position):
    ...
    async def _update_trading_statistics(self) -> None:
        ...
        self._trading_statistics['final_profit'] = calc(self._trading_statistics['in_out_profit'], self._trading_statistics['total_trading_fees'], '-')
    ...
    async def _update_detail_info(self, interval_time: int = 1, only_once: bool = False) -> None:
        ...
        while True:
            ...
            showing_texts += '\n\n已达成盈亏:\t\t{:<20}\t{}\n'.format(self._trading_statistics['in_out_profit'], self.symbol_name[-4:])
...
"""
    ...
    async def ticker_receiver(self, recv_ticker_data: dict) -> None:
        ...
        # internal_counting += 0
        if self._need_next_order:
            self._interval_count += 1
            if self._interval_count >= 3:
