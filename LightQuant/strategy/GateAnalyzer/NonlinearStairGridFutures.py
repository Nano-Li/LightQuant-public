# -*- coding: utf-8 -*-
# @Time : 2023/4/21 19:39
# @Author : 
# @File : NonlinearStairGridFutures.py 
# @Software: PyCharm
import time
import asyncio
import numpy as np
...
class NonlinearStairGridAnalyzerFutures(Analyzer):
    """
    ...
    async def validate_param(self, input_params: dict) -> dict:
        ...
        if symbol_exist:
            ...
            if param_valid:
                ...
                if self._x_lower_buffer_price_limit / self._x_price_abs_step < 20:
    ...
    async def param_ref_info(self, valid_params_dict: dict) -> str:
        ...
        for each_index, _ in enumerate(tmp_prices):
            if (tmp_prices[each_index] - tmp_prices[each_index + 1]) >= 2 * self.grid_price_step:
                double_step_price = tmp_prices[each_index]
                break
    ...
    async def ticker_receiver(self, recv_ticker_data: dict) -> None:
        ...
        if self._trading_statistics['final_profit'] >= 10 and self._is_trading:
