# -*- coding: utf-8 -*-
# @Time : 2023/11/29 21:19
# @Author : 
# @File : OptimalGridSeekerFutures.py 
# @Software: PyCharm
import time
import asyncio
import numpy as np
...
class OptimalGridSeekerAnalyzerFutures(Analyzer):
    """
    测试策略，用于探索最优网格间距，临时用智能调仓beta框架
    仓位资金相同时，通过拆分网格可以获得更多交易机会
    但是过密的网格也会降低套利收益，因此存在一个最优网格间距

    策略：使用一个给定时间长度(600s)的滑动窗口，统计这个区间的所有网格成交情况
         并计算相关参数做比对

    实际策略可能需要更灵活复杂的动态滑动窗口长度判断
    """
    ...
    def __init__(self) -> None:
        ...
        # === 策略新增 === # 全局最低网格价格，存在过的最低网格价格
        self.overall_bottom_price = 0
        ...
        # 修正仓位功能 # 修正仓位订单id，-1代表当前没有需要修正的挂单
        ...
        # ============ 核心策略功能 ============ # 使用双端队列，保存价格信息并做统计
        self._public_trade_prices = deque()
        # [{timestamp: price}, ]    [{171654694: 38000.1}, ]
        self._over_time_trade_prices = deque(maxlen=1000)  # 指定最大长度

        self._ticker_prices = deque()
        # 市场波动率的参考时间，一般认为 600 s
        self.back_trace_time = 600

        # 尾端价格，用于减少累计统计数据
        self.current_drop_price = 0
        self.previous_drop_price = 0

        # 存储所有不同间距网格的网格价格，其中key(int) 表示该网格的网格间距为精度的n倍
        self.all_grids: dict[int, tuple[float | int]] = {}
        # 存储所有网格间距
        self.all_price_steps: dict[int, float] = {}
        # 存储所有补仓网格数量
        self.all_filling_grid_num: dict[int, int] = {}
        # 存储所有c index
        self.all_critical_indices: dict[int, int] = {}
        # 存储所有drop index, 用于更新统计数据用
        self.all_drop_indices: dict[int, int] = {}

        # 价格增减多少倍精度，取时间范围内的平均值
        self.risen_price_precision: int = 0
        self.dropped_price_precision: int = 0

        # 每个网格的统计数据, {{1: {...}}, {2: {...}}}
        self.all_grid_statistics: dict[int, dict] = {}
    ...
    async def validate_param(self, input_params: dict, acquire_params: bool = True) -> dict:
        """
        该策略下，自动填充默认参数，输入参数无意义
        :param input_params:
        :param acquire_params:
        :return:
        """
        ...
        if symbol_exist:
            ...
            #     param_valid = False
            self._x_leverage = min(self.symbol_max_leverage, 20)
            ...
            if param_valid:
                # 自动设置上界为涨50%
                self._x_grid_up_limit = round_step_size(self.current_symbol_price * 1.5, self.symbol_price_min_step)
            ...
            if param_valid:
                ...
                self._x_price_abs_step = self.symbol_price_min_step
                ...
                self._x_filling_price_step = round_step_size(self.current_symbol_price * 0.05, self.symbol_price_min_step)
                self._x_lower_price_step = round_step_size(self.current_symbol_price * 0.30, self.symbol_price_min_step)
            ...
            # 缓冲部分
            if param_valid:
                self._x_max_grid_step = round_step_size(self._x_price_abs_step * 2, self.symbol_price_min_step)
                self._x_lower_buffer_price_limit = round_step_size(self._x_max_grid_step * 3, self.symbol_price_min_step)
    ...
    def confirm_params(self, input_params: dict) -> None:
        ...
        self._my_executor.start_single_contract_public_trade_subscription(self.symbol_name)
    ...
    def derive_functional_variables(self, assign_entry_price: float = 0) -> None:
        ...
        self.overall_bottom_price = self.lower_step_price
        self.up_grid_num = int(calc(calc(self._x_grid_up_limit, self.entry_grid_price, '-'), self.grid_price_step, '/')) + 1

        # === 核心功能 === # 此处开始计算所有网格价格
        max_grid_price_step = round_step_size(self.entry_grid_price * 0.01, self.symbol_price_min_step)
        max_grid_multiplier = int(calc(max_grid_price_step, self.symbol_price_min_step, '/'))
        # 首先得到所有网格间距乘数
        set_grids_num = 30
        if max_grid_multiplier <= set_grids_num:
            self.all_price_steps = {
                i: calc(i, self.symbol_price_min_step, '*') for i in range(1, max_grid_multiplier + 1)
            }
        else:
            # 给定一些特定比例, 万1，万5，万8，千1，千2，千3，千5，千8
            set_ratios = [0.0001, 0.0002, 0.0003, 0.0004, 0.0005, 0.0006, 0.0008, 0.0009, 0.001, 0.0015, 0.002, 0.0025, 0.003, 0.004, 0.005, 0.008]
            set_multipliers = [int(calc(round_step_size(self.entry_grid_price * each_ratio, self.symbol_price_min_step, upward=True), self.symbol_price_min_step, '/'))
                               for each_ratio in set_ratios]
            set_multipliers = sorted(set(set_multipliers))  # 剔除重复整数
            all_using_multipliers = sorted(set([1, 2, 3, 4, 5] + sorted(set(round(5 + i * (max_grid_multiplier - 5) / set_grids_num) for i in range(1, set_grids_num + 1)))))

            print("原始numbers: ", all_using_multipliers)
            for m in set_multipliers:
                if all_using_multipliers[0] <= m <= all_using_multipliers[-1]:
                    # 找到numbers中最接近且不大于m的元素的索引
                    index = next((i for i, x in enumerate(all_using_multipliers) if x >= m), len(all_using_multipliers))
                    if index < len(all_using_multipliers):
                        all_using_multipliers[index] = m
                        # print(f"替换元素: {m} 到位置 {index}")
            print("修改后的numbers: ", all_using_multipliers)

            self.all_price_steps = {
                i: calc(i, self.symbol_price_min_step, '*') for i in all_using_multipliers
            }

        # 顺便初始化indices
        self.all_critical_indices = {key: int(0) for key in self.all_price_steps}
        self.all_drop_indices = self.all_critical_indices.copy()
        self.all_filling_grid_num = self.all_critical_indices.copy()

        # 得到所有网格价格
        self.all_grids = {}
        self.all_grid_statistics = {}
        for each_key in self.all_price_steps.keys():
            each_price_step = self.all_price_steps[each_key]
            filling_grid_step_num = int(self._x_filling_price_step / each_price_step)
            lower_grid_step_num = int(self._x_lower_price_step / each_price_step)
            self.all_filling_grid_num[each_key] = filling_grid_step_num

            self.all_grids[each_key] = tuple([calc(self.lower_step_price, calc(i, each_price_step, '*'), '+')
                                              for i in range(1 + lower_grid_step_num + self.upper_buffer_steps_num * filling_grid_step_num)])
            # 顺便指定c index
            self.all_critical_indices[each_key] = lower_grid_step_num
            # 顺便定义统计数据
            self.all_grid_statistics[each_key] = self.Single_grid_statistic.copy()

        self.all_drop_indices = self.all_critical_indices.copy()
        print(self.all_critical_indices)
        print(self.all_grids.keys())
        ...
        self.all_grid_price = self.all_grids[1]
        self.grid_total_num = len(self.all_grids[1])
        ...
        self.present_step_up_price = calc(self.entry_grid_price, self.filling_price_step, '+')
        self.lower_step_price = calc(self.entry_grid_price, self._x_lower_price_step, '-')
        print('entry price: {}, filling_price_step: {}'.format(self.entry_grid_price, self.filling_price_step))

        self.entry_index = self.all_critical_indices[1]
    ...
    def stop(self) -> None:
        if self._is_waiting:
            ...
            self._my_executor.stop_single_contract_public_trade_subscription(self.symbol_name)
    ...
    async def _start_strategy(self):
        ...
        self.previous_drop_price = self.current_drop_price = self.current_symbol_price
        # 如果直接入场，在此得到策略参数
        self.derive_functional_variables()
    ...
    def _grid_stair_step_up(self) -> None:
        ...
        for each_key in self.all_grids.keys():
            each_price_step = self.all_price_steps[each_key]
            filling_grid_step_num = self.all_filling_grid_num[each_key]

            end_price = self.all_grids[each_key][-1]
            add_grid_price = tuple(calc(end_price, calc(each_price_step, i, '*'), '+') for i in range(1, filling_grid_step_num + 1))

            self.all_grids[each_key] += add_grid_price
        ...
        self.lower_step_price = calc(self.lower_step_price, self.filling_price_step, '+')
    ...
    async def _update_trading_statistics(self) -> None:
        ...
        :return:
        """
        for each_key, each_statistic in self.all_grid_statistics.items():
            each_statistic['matched_pair_num'] = min(each_statistic['filled_buy_order_num'], each_statistic['filled_sell_order_num'])
            each_statistic['matched_profit'] = calc(calc(each_statistic['matched_pair_num'], self.all_price_steps[each_key], '*'),
                                                    calc(each_key, self.symbol_quantity_min_step, '*'), '*')
            if max(each_statistic['filled_buy_order_num'], each_statistic['filled_sell_order_num']) == 0:
                each_statistic['effective_ratio'] = 0
            else:
                each_statistic['effective_ratio'] = round(each_statistic['matched_pair_num'] /
                                                          max(each_statistic['filled_buy_order_num'], each_statistic['filled_sell_order_num']) * 100, 1)
            each_statistic['is_effective'] = True if each_statistic['effective_ratio'] >= 40 and each_statistic['matched_pair_num'] >= 10 else False
            each_statistic['n0s0^2'] = int(each_statistic['matched_pair_num'] * int(each_key ** 2))
    ...
    async def _update_detail_info(self, interval_time: int = 3, only_once: bool = False) -> None:
        ...
        while True:
            ...
            showing_texts += '-' * 58
            showing_texts += '\n\n市场波动率评估:\t总/涨/跌\t{}/{}/{}\n\n'.format(
                self.risen_price_precision + self.dropped_price_precision, self.risen_price_precision, self.dropped_price_precision)
            showing_texts += '-' * 58
            showing_texts += '\n\n间距倍率\t补仓网格数\t网格间距\t间距比例\t卖/买 成交\t成交对数\t有效比率\t有效\t交易量\t套利收益\tn₀S₀²\t最优\n'.format(
                False if self.adjusting_index == -1 else True)
            max_key = max(self.all_grid_statistics, key=lambda x: self.all_grid_statistics[x]['n0s0^2'])
            for each_key, each_statistic in self.all_grid_statistics.items():
                showing_texts += '\n{:>6}\t{}\t{}\t{} %\t{}/{}\t{}\t{} %\t{}\t{}\t{}\t{}\t{}'.format(
                    'x' + str(each_key), self.all_filling_grid_num[each_key], self.all_price_steps[each_key],
                    round(self.all_price_steps[each_key] / self.current_symbol_price * 100, 3), each_statistic['filled_buy_order_num'], each_statistic['filled_sell_order_num'],
                    each_statistic['matched_pair_num'], each_statistic['effective_ratio'], '■' if each_statistic['is_effective'] else '□',
                    round(each_statistic['total_trade_volume'], 2), each_statistic['matched_profit'], each_statistic['n0s0^2'], '■' if each_key == max_key else '□'
                )
    ...
    async def temp_log_thinking(self):
        """
        实现方法：
        在导出网格间距时，导出所有的需要统计的网格间距，智能使用大概20个，从x1到1.5%，并保存
        由于使用了智能判断，因此使用字典{5: [...all prices]}保存
        同时同步保存一个index字典，{5: 58} 用于保存所有c index
        再使用一个保存字典，{5: 34} 用于保存所有的尾端drop index，更新统计数据用

        头部增加价格信息：
        由于数据链性质，使用public trade 更新最新价格即可
        每次获得最新的价格信息，使用maintainer by price，
        并判断价格是升高还是下降
        如果价格升高，则对于每一个网格，向上遍历价格判断成交数量，计算交易量
        如果价格下降，则反之，如果价格不变，则不做任何判断

        尾端减少价格信息：
        丢掉一个价格信息，根据 d index遍历字典价格，并相应减少统计信息

        需要使用一个栈，来存储最近10min内的价格信息，websocket新收到价格信息时，保存到栈中，超出10min的移动到另一个垃圾桶中
        后续真实策略，考虑使用文件保存，否定
        使用deque保存价格栈，并限定备用栈的长度
    ...
    async def _add_trade_data(self) -> None:
        """
        根据新获得的最新成交价格，判断所有网格交易情况，不考虑价格超出下边界的情况，因为该函数不会被调用
        操作：
            1.如果价格升高，从c index上一个价格开始遍历，在价格变化范围内的网格价格成交，价格相等视为成交
                更新c index, 更新交易量和交易数量
            2.如果价格降低，从c index下一个价格开始遍历
            3.如果价格不变，不操作
        对每一个网格都做出如上描述的计算
        :return:
        """
        if self.current_symbol_price > self.previous_price:
            self.risen_price_precision += int(calc(calc(self.current_symbol_price, self.previous_price, '-'), self.symbol_price_min_step, '/'))
            # 操作每一个网格及数据
            for each_key, each_grid in self.all_grids.items():
                each_c_index = self.all_critical_indices[each_key]
                # 不用判断大于max index 的情况，因为网格价格一般会记录超出上边界的网格价格
                while each_grid[each_c_index + 1] <= self.current_symbol_price:
                    self.all_grid_statistics[each_key]['filled_sell_order_num'] += 1
                    self.all_grid_statistics[each_key]['total_trade_volume'] = \
                        calc(self.all_grid_statistics[each_key]['total_trade_volume'],
                             calc(calc(each_grid[each_c_index + 1], self.symbol_quantity_min_step, '*'), each_key, '*'), '+')
                    # 往上遍历网格价格
                    each_c_index += 1
                # 最后更新c index
                if self.all_critical_indices[each_key] != each_c_index:
                    self._log_info('{:>6}  网格\t成交了   {:<4} 个卖单'.format('x' + str(each_key), each_c_index - self.all_critical_indices[each_key]))
                    self.all_critical_indices[each_key] = each_c_index

        elif self.current_symbol_price < self.previous_price:
            self.dropped_price_precision += int(calc(calc(self.previous_price, self.current_symbol_price, '-'), self.symbol_price_min_step, '/'))

            for each_key, each_grid in self.all_grids.items():
                each_c_index = self.all_critical_indices[each_key]
                # 不用判断等于0 index 的情况，因为下边界价格不会调用该函数
                while each_grid[each_c_index - 1] >= self.current_symbol_price:
                    self.all_grid_statistics[each_key]['filled_buy_order_num'] += 1
                    self.all_grid_statistics[each_key]['total_trade_volume'] = \
                        calc(self.all_grid_statistics[each_key]['total_trade_volume'],
                             calc(calc(each_grid[each_c_index - 1], self.symbol_quantity_min_step, '*'), each_key, '*'), '+')
                    # 往下遍历网格价格
                ...
                # 最后更新c index
                if self.all_critical_indices[each_key] != each_c_index:
                    self._log_info('{:>6}  网格\t成交了   {:<4} 个买单'.format('x' + str(each_key), self.all_critical_indices[each_key] - each_c_index))
                    self.all_critical_indices[each_key] = each_c_index

        else:
            pass

    async def _drop_trade_data(self) -> None:
        """
        更新网格交易情况的反向操作版，丢掉超出时间长度的历史数据
        操作：
            1.如果价格升高，说明之前这个价格范围内的卖单成交了，丢掉相应的交易累计数据
            2.降低则反之
            3.不变则不操作
        :return:
        """
        if self.current_drop_price > self.previous_drop_price:
            self.risen_price_precision -= int(calc(calc(self.current_drop_price, self.previous_drop_price, '-'), self.symbol_price_min_step, '/'))
            # 操作每一个网格及数据
            for each_key, each_grid in self.all_grids.items():
                each_d_index = self.all_drop_indices[each_key]
                while each_grid[each_d_index + 1] <= self.current_drop_price:
                    self.all_grid_statistics[each_key]['filled_sell_order_num'] -= 1
                    self.all_grid_statistics[each_key]['total_trade_volume'] = \
                        calc(self.all_grid_statistics[each_key]['total_trade_volume'],
                             calc(calc(each_grid[each_d_index + 1], self.symbol_quantity_min_step, '*'), each_key, '*'), '-')
                    # 往上遍历网格价格
                    each_d_index += 1
                # 最后更新d index
                if self.all_drop_indices[each_key] != each_d_index:
                    self._log_info('删除数据: {:>6}  网格\t成交了   {:<4} 个卖单'.format('x' + str(each_key), each_d_index - self.all_drop_indices[each_key]))
                    self.all_drop_indices[each_key] = each_d_index

        elif self.current_drop_price < self.previous_drop_price:
            self.dropped_price_precision -= int(calc(calc(self.previous_drop_price, self.current_drop_price, '-'), self.symbol_price_min_step, '/'))
            # 操作每一个网格及数据
            for each_key, each_grid in self.all_grids.items():
                each_d_index = self.all_drop_indices[each_key]
                while each_grid[each_d_index - 1] >= self.current_drop_price:
                ...
                # 最后更新d index
                if self.all_drop_indices[each_key] != each_d_index:
                    self._log_info('删除数据: {:>6}  网格\t成交了   {:<4} 个买单'.format('x' + str(each_key), self.all_drop_indices[each_key] - each_d_index))
                    self.all_drop_indices[each_key] = each_d_index

        else:
            pass

    async def _additional_action_task_old(self) -> None:
        """
    ...
    async def public_trade_receiver(self, recv_trade_data: dict) -> None:
        """
        使用栈数据结构保存最近盘口成交价格
        :param recv_trade_data:
        :return:
        """
        self.previous_price = self.current_symbol_price
        self.current_symbol_price = recv_trade_data['price']
        ...
        if self._is_trading:
            ...
            await self._maintainer_by_price()
            if self.current_symbol_price >= self._x_grid_up_limit:
                self._log_info('最新成交价格超出上边界!需要终止策略')
                asyncio.create_task(self._terminate_trading(reason='达到网格上边界'))
                return

            trade_time_stamp = recv_trade_data['event_time']
            price_data = {trade_time_stamp: self.current_symbol_price}

            # self._log_info('更新时间 {}\t成交价格 {}'.format(str(pd.to_datetime(trade_time_stamp, unit='ms') + pd.Timedelta(hours=8))[10:], self.current_symbol_price))

            # 添加价格数据
            self._public_trade_prices.append(price_data)

            # 更新网格数据，只有在网格边界只上才更新
            if self.current_symbol_price > self.lower_step_price:
                await self._add_trade_data()

            # 末端删除超过时间的数据, 1min = 60000
            while trade_time_stamp - next(iter(self._public_trade_prices[0])) > 21600000:
                over_time_price_data = self._public_trade_prices.popleft()
                self._over_time_trade_prices.append(over_time_price_data)

                # self._log_info('删除时间数据 {}'.format(over_time_price_data))

                self.previous_drop_price = self.current_drop_price
                self.current_drop_price = next(iter(over_time_price_data.values()))

                # 删除过时的网格累计数据
                # todo: 目前无法处理在中途，价格跌破下区间并上升的情况，因为不知道600s之前的价格是否在套利区间范围内，
                #  因此，默认设置很大的回调区间以确保虚拟网格一直在交易
                if self.previous_drop_price > self.overall_bottom_price:
                    await self._drop_trade_data()
    ...
    # @staticmethod
    # def grid_fully_traded() -> bool:
    #     # 价格和网格价格相等时，随机选择该网格是否成交
