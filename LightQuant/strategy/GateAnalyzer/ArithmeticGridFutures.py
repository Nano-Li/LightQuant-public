...
class ArithmeticGridAnalyzerFutures(Analyzer):
    """
    等差网格策略类，对接合约
    """
    STG_NAME = '等差网格'
    ...
    def __init__(self) -> None:
        ...
        # 持仓为 0 的index
        self._zero_pos_index = 0        # todo: 退役
        ...
        self.grid_qty_per_order = None  # 注意：在gate合约中，该数量表示合约张数，实际交易数量为 张数*最小数量间距
    ...
    async def validate_param(self, input_params: dict) -> dict:
        ...
        leverage = validated_params['leverage']     # todo: 杠杆合理化检测，数值范围
        ...
        if symbol_exist:
            ...
            each_grid_qty = round_step_size(each_grid_qty, 1)
        ...
        validated_params['leverage'] = leverage
    ...
    async def param_ref_info(self, valid_param_dict: dict) -> str:
        ...
        each_grid_qty = calc(valid_param_dict['each_grid_qty'], self.symbol_quantity_min_step, '*')     # 此处使用的是真实数量
        ...
        info_texts += '*** {} ***\n'.format('=' * 32)
        # current_symbol_price = await self._running_loop.create_task(self._my_executor.get_current_price(symbol_name))
        current_symbol_price = await self._my_executor.get_current_price(symbol_name)
        # print('current price = {}\n'.format(current_symbol_price))
        info_texts += '\n当前时间: {}\n'.format(str(pd.to_datetime(self.gen_timestamp(), unit='ms')))
        info_texts += '\n合约名称: {}\t\t\t当前价格: {}\n'.format(symbol_name, str(current_symbol_price))
        # todo: 此处更新了 critical index, 不对称, 解决方法: 与paramwidgets有统一的paramdict, 即可解决问题
        ...
        ini_order_quantity, ini_position_qty = 0, 0
        # 保证金占用为 0 时的 index
        ...
        if ini_ratio > 0:
            if valid_param_dict['grid_side'] == self.BUY:
                ...
                # info_texts += '网格方向输入错误\n'
                pass
        ini_margin_cost = calc(calc(ini_order_quantity, current_symbol_price, '*'), leverage, '/')
        # 计算可能的最大保证金占用      todo: 实际上账户要求的最大资金比这个高，因为出现亏损时账户的可用资金会减少，可用资金大于保证金时才能继续挂单
        # max_position_qty_buy, max_position_qty_sell = ini_position_qty, ini_position_qty
        max_position_qty_buy = calc(len(all_grid_price[1:zero_margin_index]), each_grid_qty, '*')
        max_position_qty_sell = calc(len(all_grid_price[zero_margin_index + 1:-1]), each_grid_qty, '*')

        max_margin_cost_buy = calc(calc(max_position_qty_buy, all_grid_price[-1], '*'), leverage, '/')
        max_margin_cost_sell = calc(calc(max_position_qty_sell, all_grid_price[0], '*'), leverage, '/')

        # 计算最大亏损参考，相当于计算上下边界的未配对盈亏(相反数)
        # max_buy_loss, max_sell_loss = 0, 0
        max_buy_loss = - self.unmatched_profit_calc(self.critical_index, 0, all_grid_price, each_grid_qty, current_symbol_price, ini_position_qty)
        max_sell_loss = - self.unmatched_profit_calc(self.critical_index, -1, all_grid_price, each_grid_qty, current_symbol_price, ini_position_qty)

        # buy_order_prices, sell_order_prices = all_grid_price[:self.critical_index], all_grid_price[self.critical_index + 1:]
        # for each_price in buy_order_prices:
        #     # max_position_qty_buy = calc(max_position_qty_buy, each_grid_qty, '+')
        #     max_buy_loss = calc(calc(calc(each_price, all_grid_price[0], '-'), each_grid_qty, '*'), max_buy_loss, '+')
        # for each_price in sell_order_prices:
        #     # max_position_qty_sell = calc(max_position_qty_sell, each_grid_qty, '+')
        #     max_sell_loss = calc(calc(calc(all_grid_price[-1], each_price, '-'), each_grid_qty, '*'), max_sell_loss, '+')
        # max_buy_loss = calc(calc(calc(current_symbol_price, all_grid_price[0], '-'), ini_position_qty, '*'), max_buy_loss, '+')
        # max_sell_loss = calc(calc(calc(current_symbol_price, all_grid_price[-1], '-'), ini_position_qty, '*'), max_sell_loss, '+')
        ...
        info_texts += '\n初始保证金占用\t{:<8}\t{:<8}\n'.format(str(ini_margin_cost), symbol_name[-4:])
        if max_margin_cost_buy > max_margin_cost_sell:
            info_texts += '最大保证金占用\t{:<8}\t{:<8}\t该情况发生于网格 下边界\n'.format(str(max_margin_cost_buy), symbol_name[-4:])
        elif max_margin_cost_buy < max_margin_cost_sell:
            info_texts += '最大保证金占用\t{:<8}\t{:<8}\t该情况发生于网格 上边界\n'.format(str(max_margin_cost_sell), symbol_name[-4:])
        else:
            info_texts += '最大保证金占用\t{:<8}\t{:<8}\t该情况发生于网格 双边边界\n'.format(str(max_margin_cost_buy), symbol_name[-4:])

        if max_buy_loss > max_sell_loss:
            info_texts += '\n参考最大亏损\t{:<8}\t{:<8}\t当价格单边冲出 下边界\n'.format(str(max_buy_loss), symbol_name[-4:])
        elif max_buy_loss < max_sell_loss:
            info_texts += '\n参考最大亏损\t{:<8}\t{:<8}\t当价格单边冲出 上边界\n'.format(str(max_sell_loss), symbol_name[-4:])
        else:
            info_texts += '\n参考最大亏损\t{:<8}\t{:<8}\t当价格单边冲出 双边边界\n'.format(str(max_buy_loss), symbol_name[-4:])
    ...
    async def _start_strategy(self):
        ...
        asyncio.create_task(self._my_executor.change_symbol_leverage(self.symbol_name, self.account_leverage))
        # 开启数据统计
    ...
    async def _layout_net(self):
        ...
        if self.ini_trade_ratio > 0:
            ...
            self._log_info('市价下单 {} 张'.format(ini_order_quantity))
            ...
            self._initial_position_qty = calc(ini_order_quantity, self.symbol_quantity_min_step, '*')
            adding_volume = calc(calc(ini_order_quantity, self.symbol_quantity_min_step, '*'), current_symbol_price, '*')
            ...
            print('init order qty = {} 张'.format(ini_order_quantity))
    ...
    async def _maintain_grid_order(self, filled_order_id: str, append_info: str = None) -> None:
        ...
        if this_order_index == 0:
            if self.low_boundary_stop:
                ...
                adding_volume = calc_sum([calc(each_price, calc(self.grid_qty_per_order, self.symbol_quantity_min_step, '*'), '*') for each_price in
    ...
    async def _update_trading_statistics(self):
        ...
        self._trading_statistics['matched_profit'] = calc(calc(filled_trading_pair, self.grid_price_step, '*'),
                                                          calc(self.grid_qty_per_order, self.symbol_quantity_min_step, '*'), '*')
        # 手算的，使用于等差网格
        ...
        unmatched_2 = calc(unmatched_2, calc(calc(self.grid_qty_per_order, self.symbol_quantity_min_step, '*'), self.grid_price_step, '*'), '*')
        ...
        # if self.grid_side == self.BUY:
        #     unmatched_1 = int(calc((self._zero_pos_index - self._initial_index + 1), (self._zero_pos_index - self._initial_index), '*') / 2)
        # elif self.grid_side == self.SELL:
        #     unmatched_1 = int(calc((self._initial_index - self._zero_pos_index + 1), (self._initial_index - self._zero_pos_index), '*') / 2)
        # else:
        #     unmatched_1 = 0
        # unmatched_2 = int(calc(abs(self._zero_pos_index - self.critical_index), (abs(self._zero_pos_index - self.critical_index) - 1), '*') / 2)
        # self._trading_statistics['unmatched_profit'] = calc(calc((unmatched_1 - unmatched_2), self.grid_price_step, '*'),
        #                                                     calc(self.grid_qty_per_order, self.symbol_quantity_min_step, '*'), '*')
    ...
    async def _terminate_trading(self, reason: str = '') -> None:
        ...
        close_cmd['quantity'] = self.symbol_max_mkt_qty     # todo: important!!!!!!!!!!!!!!!!! 这个一直是0
    ...
    async def _update_detail_info(self, interval_time: int = 1, only_once: bool = False) -> None:
        ...
        while True:
            ...
            # ------------------------------------------------
            #
            # 已达成交易量:		221.45400000000004 USDT
            #
            # 已实现套利:		0.0      USDT
            #
            # 未配对盈亏:		-1.219   USDT
    ...
    async def _output_statistics(self, interval_time: int = 900) -> None:
        ...
        :param interval_time: 每次输出统计的时间间隔，默认 10 min
        :return:
        """
        if self._first_back:
        ...
        while True:
            filled_trading_pair = min(self._trading_statistics['filled_buy_order_num'], self._trading_statistics['filled_sell_order_num'])
            filled_profit = calc(calc(filled_trading_pair, self.grid_price_step, '*'), calc(self.grid_qty_per_order, self.symbol_quantity_min_step, '*'), '*')
            self._log_info('\n\n*** {} ***'.format('=' * 51))
            self._log_info('交易统计:\t\t当前时间 {}'.format(str(str(pd.to_datetime(self.gen_timestamp(), unit='ms')))))
            self._log_info('-' * 36)
            self._log_info('买单挂单成交次数:\t{:<10}'.format(str(self._trading_statistics['filled_buy_order_num'])))
            self._log_info('卖单挂单成交次数:\t{:<10}'.format(str(self._trading_statistics['filled_sell_order_num'])))
            self._log_info('总成交次数:\t\t{:<10}'.format(str(self._trading_statistics['filled_buy_order_num'] + self._trading_statistics['filled_sell_order_num'])))
            self._log_info('-' * 36)
            self._log_info('已达成交易量:\t\t{:<8} {}'.format(str(self._trading_statistics['achieved_trade_volume']), self.symbol_name[-4:]))
            # todo: 更改为净利润，计算手续费
            self._log_info('已实现套利利润:\t{:<8} {}'.format(filled_profit, self.symbol_name[-4:]))
            self._log_info('*** {} ***\n\n'.format('=' * 51))
            await asyncio.sleep(interval_time)
    ...
    async def report_receiver(self, recv_data_dict: dict, append_info: str = None) -> None:
        ...
        if recv_data_dict['status'] == Token.ORDER_FILLED:
            ...
            # todo: log this
            self._log_info('收到挂单成功信息\t\t价格: {:<12}\tid: {:<10}'.format(recv_data_dict['price'], recv_data_dict['id']))
            pass
        elif recv_data_dict['status'] == Token.POC_SUCCESS:
            self._log_info('收到poc挂单成功信息\t\t价格: {:<12}\tid: {:<10}'.format(recv_data_dict['price'], recv_data_dict['id']))
            pass
